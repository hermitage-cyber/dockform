...
describe("penalty_contract_daily_percent", () => {
  it("кейс п.8.2 шаблона: 100 000 ₽ × 0,3% × 15 дней", () => {
    ...
    // 100000 × 0.003 = 300/день, × 15 = 4500 — формат «1 234,56».
    expect(r["пеня_в_день"]).toBe("300,00");
    // toLocaleString разделяет тысячи неразрывным пробелом U+00A0 — это
    // правильно для документа (Word не перенесёт сумму между строками).
    expect(r["сумма_неустойки"]).toBe("4 500,00");
